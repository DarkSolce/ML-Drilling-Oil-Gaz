...
class DataLoader:
    def load_formation_data(self):
        # Generate synthetic formation pressure data
        np.random.seed(42)
        n_samples = 2000
        ...
        # Create synthetic formation pressure based on depth and other factors
        data['FPress'] = (0.433 * data['WellDepth'] + 
                         0.1 * data['WBoPress'] + 
    ...
    def load_kick_data(self):
        # Generate synthetic kick detection data
        np.random.seed(43)
        n_samples = 1500
...
class DataPreprocessor:
    def prepare_formation_pressure_data(self, data):
        # Simple preprocessing - remove nulls and outliers
        processed = data.copy()
        ...
        for col in processed.columns:
            if processed[col].dtype in ['float64', 'int64']:
                ...
                processed = processed[
                    (processed[col] >= mean - 3*std) & 
                    (processed[col] <= mean + 3*std)
                ]
        
        return processed
    
    def prepare_kick_detection_data(self, data):
        return self.prepare_formation_pressure_data(data)
...
class MockModel:
    ...
    def train(self, X, y=None, **kwargs):
        ...
        if y is not None:
            # Regression metrics
            return {
                'val_r2': np.random.uniform(0.7, 0.95),
                'val_rmse': np.random.uniform(50, 150),
                'val_mae': np.random.uniform(30, 100),
                'explained_variance_ratio': np.random.uniform(0.6, 0.9)
            }
        else:
            # Anomaly detection metrics
            return {
                'val_anomaly_rate': np.random.uniform(0.01, 0.05),
                'spe_threshold': np.random.uniform(5, 15),
                'explained_variance_ratio': np.random.uniform(0.8, 0.95)
            }
    ...
    def predict(self, X):
        ...
        if hasattr(self, 'is_anomaly_detector') and self.is_anomaly_detector:
            # Return anomaly predictions (0 or 1)
            return np.random.choice([0, 1], size=len(X), p=[0.95, 0.05])
        else:
            # Return regression predictions
            return np.random.normal(2000, 300, len(X))
    ...
    def get_feature_importance(self, top_k=10):
        ...
        importance = {col: np.random.uniform(0, 1) for col in self.feature_columns[:top_k]}
        # Sort by importance
        return dict(sorted(importance.items(), key=lambda x: x[1], reverse=True))
    
    def save_model(self, filepath):
        # Mock save functionality
        return f"models/{filepath}"

# Model classes
class PCRFormationPressure(MockModel):
    def __init__(self, n_components=4):
        super().__init__("PCR")
        self.n_components = n_components

class XGBoostFormationPressure(MockModel):
    def __init__(self):
        super().__init__("XGBoost")

class EnsembleFormationPressure(MockModel):
    def __init__(self, models):
        super().__init__("Ensemble")

class PCAKickDetection(MockModel):
    def __init__(self, variance_threshold=0.9):
        super().__init__("PCA")
        self.variance_threshold = variance_threshold
        self.is_anomaly_detector = True

class EnsembleKickDetection(MockModel):
    def __init__(self, models):
        super().__init__("Ensemble")
        self.is_anomaly_detector = True
    ...
    def render_model_performance_page(self):
        ...
        if kick_models:
            ...
            if kick_metrics:
                ...
                with col2:
                    st.info("Additional metrics available with labeled data")
