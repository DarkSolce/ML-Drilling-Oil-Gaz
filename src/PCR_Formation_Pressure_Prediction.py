# -*- coding: utf-8 -*-
"""
Created on Sun Jun 30 16:12:40 2024
...
df2 = df1[['FPress', 'WellDepth', 'BTBR', 'WBoPress', 'HLoad', 'WoBit', 'RoPen', 'DPPress']]
...
Xstd = StandardScaler().fit_transform(X2[:, :])
...
# =============================================================================
# xticks = np.arange(1, 7).astype('uint8')
...
with plt.style.context('ggplot'):
    ...
    ax.plot(y, z[1] + z[0]*y, c='blue', linewidth=1, label='Predicted regression line')
    ax.plot(y, y, color='green', linewidth='1', label='Expected regression line')
    rpd = y.std()/np.sqrt(mscv)
    plt.title('$R^{2}$ (CV): %0.4f, RPD: %0.4f'%(r2cv, rpd))
    plt.legend()
    plt.xlabel('Actual')
    plt.ylabel('Predicted')
    plt.savefig(r'C:\a_PhD Research\SoftSensor\Figures\Scientific data Figs\PCR.svg', bbox_inches="tight")
    plt.show()
